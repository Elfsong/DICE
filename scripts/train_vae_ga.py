...
vae_config = {
    "encoder": {
        "model_name": wandb.config["encoder_model"],
        "tokenizer_name": wandb.config["encoder_model"],
        "hidden_size": wandb.config["encoder_hidden_size"],
        "latent_size": wandb.config["latent_size"],
    },
    "decoder": {
        "model_name":  wandb.config["decoder_model"],
        "tokenizer_name":  wandb.config["decoder_model"],
...
# Load Model
print("Load Model (it can be slow, be patient)")
...
# Optimizer & Scheduler
print("Load Optimizer & Scheduler")
...
for epoch in range(wandb.config["epochs"]):
    ...
    for index, batch in enumerate(train_dataloader):
        ...
        total_loss = reconstruction_loss.mean() + kl_loss * cyclical_annealing * wandb.config["kl_scale"]

        total_loss = total_loss / wandb.config["gradient_accumulation_steps"] 
        ...
        # Weights Update
        if ((index + 1) % wandb.config["gradient_accumulation_steps"] == 0) or (index + 1 == len(train_dataloader)):
            optimizer.step()
            optimizer.zero_grad()
